   ...
   2> 값
      ...
      3] 데이터 타입
        ...
        4} 용어 정리
          ...
          8) 데이터 타입(Data Type) : 프로그래밍 언어에서 사용할 수 있는 값의 종류

          9) 변수(Variable) : 값이 저장된 메모리 공간의 주소를 가리키는 식별자(identifier)

          10) 리터럴(literal) : 소스코드 안에서 직접 만들어 낸 상수 값 자체를 말하며 값을 구성하는 최소 단위
...
3. 데이터 타입과 변수
   ...
   1] 데이터 타입
        ...
        0} 들어가기에 앞서...
          ...
          2) 코드에서 사용되는 모든 데이터는 메모리에 저장하고 참조할 수 있어야 한다.

        1} 원시 타입(Primitive Data Type)

          3) 원시 타입의 값 : 변경 불가능한 값(immutable value) & 값에 의한 전달(pass-by-value)
   ...
   2] 변수
        ...
        0} 들어가기에 앞서...
          ...
          11) 정의 : 프로그램에서 사용되는 데이터를 일정 기간 동안 기억하여 필요한 때에 다시 사용하기 위해 데이터에 고유의 이름인 식별자(identifier)를 명시한 것
   
          12) 변수명 : 변수에 명시한 고유한 식별자
   
          13) 변수값 : 변수로 참조할 수 있는 데이터
   
          14) 식별자 : 어떤 대상을 유일하게 식별할 수 있는 이름
         -> 식별자에는 변수명, 함수명, 프로퍼티명, 클래스명 등이 있다.

          15) 변수는 키워드(var, let, const)를 사용하여 선언하고 할당 연산자를 사용해 값을 할당하며 식별자인 변수명을 사용해 변수에 저장된 값을 참조한다.

        1} 변수의 선언

          16) 변수명(식별자(identifier)) 명명 규칙
        ...
        2} 변수의 중복 선언
          ...
          18) var 키워드로 선언한 변수는 중복 선언이 가능하다.
   
          19) 부작용 : 동일한 변수명이 선언되어 있는 것을 모르고 변수를 중복 선언했다면 의도치 않게 변수의 값을 변경한다.
          -> 사용하지 않는 것이 좋다.

        3} 동적 타이핑(Dynamic Typing)

          20) 정의 : 같은 변수에 여러 타입의 값을 할당할 수 있다.
...
4. 연산자
   ...
   4] 산술 연산자
     ...
     0} 들어가기에 앞서...
       ...
       9) 정의 : 피연산자를 대상으로 수학적 계산을 수행해 새로운 숫자 값을 만든다.
   
       10) 산술 연산을 할 수 없는 경우 : NaN을 반환하다.

     1} 이항 산술 연산자

       11) 연산 : 2개의 피연산자를 대상으로 연산하여 숫자 타입의 값을 만든다.

       12) 피연산자의 값을 변경하는 부수 효과(Side effect)가 없다.
     ...
     2} 단항 산술 연산자
       ...
       14) 연산 : 1개의 피연산자를 대상으로 연산한다.
   
       15) 피연산자의 값을 변경하는 부수 효과가 있다.
       ...
       17) 전위 증가/감소 연산자(Prefix increment/decrement operator) & 후위 증가/감소 연산자(Postfix increment/decrement operator)
     ...
     3} 문자열 연결 연산자
       ...
       18) + 연산자 : 피연산자 중 하나 이상이 문자열인 경우 문자열 연결 연산자로 동작한다.
       (그 외의 경우는 덧셈 연산자로 동작한다.)

       19) + / - 단항 연산자도 암묵적 타입 변환 발생이 가능하다.
   ...
   6] 비교 연산자(Comparison Operator)
     ...
     0} 들어가기에 앞서...
       ...
       26) 반환값 : 좌항과 우항의 피연산자를 비교하여 불리언 값을 반환한다.
   
       27) 사용 : 제어문(if 문, for 문 등)의 조건식에서 주로 사용한다.
     ...
     2} 대소 관계 비교 연산자
       ...
       29) 반환값 : 피연산자의 크기를 비교하여 불리언 값을 반환한다.
   ...
   7] 삼항 조건 연산자
     ...
     -> 조건식 ? 조건식이 true일 떄 반환할 값 : 조건식이 false일 때 반환할 값
       ...
       31) 조건식 : 물음표(?) 앞의 첫번째 피연산자, 불리언 타입의 값으로 평가될 표현식
   ...
   8] 논리 연산자(Logical Operator)
     ...
     3} 논리 부정(!) 연산자
       ...
       35) 반환값 : 언제나 불리언 값을 반환한다.

       36) 피연산자 : 반드시 불리언 값일 필요는 없다.
       -> 피연산자가 불리언 값이 아니면 불리언 타입으로 암묵적 타입 변환된다.

     4} 논리합(||) 연산자와 논리곱(&&) 연산자

       37) 연산 결과 : 일반적으로 불리언 값을 반환하지만 반드시 반환해야 하는 것은 아니다.

       38) 반환 : 언제나 피연산자 중 어느 한쪽 값을 반환한다.
   ...
   11] typeof 연산자
     ...
     2} typeof 연산자
       ...
       39) 반환 : 7가지 문자열("string", "number", "boolean", "undefined", "symbol", "object", "function"(함수)) 중 하나를 반환한다.
       -> "null"을 반환하는 경우는 없다.
       ...
       41) 주의사항 : 선언하지 않은 식별자를 typeof 연산자로 연산해보면 ReferenceError가 발생하지 않고 "undefined"를 반환한다.
...
5. 제어문
   ...
   2] 조건문
     ...
     1} if..else 문
       ...
       3) 정의 : 주어진 조건식(불리언 값으로 평가될 수 있는 표현식)의 평가 결과(논리적 참, 거짓)에 따라 실행할 코드 블록을 결정한다.
       ...
       6) 코드 블록 : 코드 블록 내의 문이 하나뿐이라면 중괄홀를 생략할 수 있다.
   
       7) 대부분의 if...else 문은 연산자에서 살펴본 삼항 조건 연산자로 바꿔쓸 수 있다.

     2} switch 문

       8) 정의 : switch 문의 표현식을 평가하여 그 값과 일치하는 표현식을 갖는 case 문으로 실행 순서를 이동시킨다.
       ...
       10) 표현식 : 불리언 값보다는 문자열, 숫자 값인 경우가 많다.

       11) 사용 : 논리적 참, 거짓보다는 다양한 상황(case)에 따라 실행할 코드 블록을 결정할 때 사용한다.
   ...
   3] 반복문(Loop statement)
     ...
     1} for 문
       ...
       15) 실행 : 조건식이 거짓으로 판별될 때까지 코드 블록을 반복 실행한다.

       16) 가장 일반적으로 사용되는 반복문이다.
   ...
   4] break 문
     ...
     1} 탈출
       ...
       23) 레이블 문, 반복문(for, for...in, for...of, while, do...while) 또는 switch 문의 코드 블록을 탈출한다.

       24) 레이블 문, 반복문, switch 문의 코드 블록 이외에 break 문을 사용할 경우 : SyntaxError(문법 에러)가 발생한다.

     2} 레이블 문(Label statement)

       25) 정의 : 식별자가 붙은 문

       26) 사용 목적 : 프로그램의 실행 순서를 제어하기 위해

       27) 예시 : switch 문의 case 문과 default 문
   
       28) 탈출 : break 문에 레이블 식별자를 지정한다.
     ...
     3} 중첩된 for 문 
       ...
       30) 중첩된 for 문의 내부 for 문에서 break 문을 실행하면 내부 for 문으로 진입한다.
   ...
   5] continue 문
     ...
     1} 실행
       ...
       31) 반복문(for, for...in, for...of, while, do...while)의 코드 블록 실행을 현 지점에서 중단하고 반복문의 증감식으로 이동한다.

       32) if 문 내에서 실행해야 할 코드
...
6. 타입 변환과 단축 평가
   ...
   1] 타입 변환이란?
     ...
     2} 암묵적 타입 변환(Implicit coercion) 또는 타입 강제 변환(Type coercion)
       ...
       3) 정의 : 개발자의 의도와는 상관없이 자바스크립트 엔진에 의해 암묵적으로 타입이 자동 변환되는 것

       4) 변수 값을 재할당해서 변경하는 것이 아니라 자바스크립트 엔진이 표현식을 에러 없이 평가하기 위해 기존 값을 바탕으로 새로운 타입의 값을 만들어 단 한 번 사용하고 버린다.

       5) 드러나지 않게 타입이 자동 변환되기 때문에 타입을 변경하겠다는 개발의 의지가 코드에 명백히 나타나지 않는다.
       
       6) 타입 변환된 값으로 표현식의 평가 : 어떻게 평가될 것인지 예측 가능해야 한다.
   ...
   2] 암묵적 타입 변환
     ...
     0} 들어가기에 앞서...
       ...
       7) 자바스크립트 엔진은 표현식을 평가할 때 문맥, 즉 컨텍스트(Context)에 고려하여 암묵적 타입 변환을 실행한다.
   ...
   3] 명시적 타입 변환
     ...
     1} 문자열 타입으로 변환
       ...
       14) 문자열 타입이 아닌 값을 문자열 타입으로 변환하는 방법
     ...
     2} 숫자 타입으로 변환
       ...
       15) 숫자 타입이 아닌 값을 숫자 타입으로 변환하는 방법
     ...
     3} 불리언 타입으로 변환
       ...
       16) 불리언 타입이 아닌 값을 불리언 타입으로 변환하는 방법
   ...
   4] 단축 평가(Short-Circuit evaluation)
     ...
     1} 논리곱 연산자(&&)
       ...
       17) 반환 : 두 개의 피연산자가 모두 true로 평가될 때 true를 반환한다.

       18) 진행 : 오른쪽에서 왼쪽으로 평가가 진행된다.

     2} 논리합 연산자(||)

       19) 반환 : 두 개의 피연산자 중 하나만 true로 평가되어도 true를 반환한다.

       20) 진행 : 오른쪽에서 왼쪽으로 평가가 진행된다.
     ...
     5} 단축 평가가 유용하게 사용되는 상황
       ...
       25) 객체가 null인지 확인하고 프로퍼티를 참조할 때
...
7. 프로토타입
   1] 프로토타입 객체
     ...
     3} ECMAScript spec
       ...
       3) 자바스크립트의 모든 객체 : [[Prototype]]이라는 인터널 슬롯(internal slot)를 가진다.

       4) [[Prototype]]의 값 : null 또는 객체이며 상속을 구현하는데 사용된다.
       ...
       5) [[Prototype]] 객체의 데이터 프로퍼티 : get 액세스를 위해 상속되어 자식 객체의 프로퍼티처럼 사용할 수 있다.
       -> set 액세스는 허용되지 않는다.

       6) student 객체 : __proto__ 프로퍼티로 자신의 부모 객체(프로토타입 객체)인 Object.prototype을 가리키고 있다.
   ...
   2] [[Prototype]] vs prototype 프로퍼티
     ...
     2} [[Prototype]]
       ...
       7) 함수를 포함한 모든 객체가 가지고 있는 인터널 슬롯이다.

       8) 객체의 입장에서 자신의 부모 역할을 하는 프로토타입 객체를 가리키며 함수 객체의 경우 Function.prototype를 가리킨다.
   
     3} prototype 프로퍼티

       9) 함수 객체만 가지고 있는 프로퍼티이다.

       10) 함수 객체가 생성자로 사용될 때 이 함수를 통해 생성될 객체의 부모 역할을 하는 개체(프로토타입 객체)를 가리킨다.
   ...
   4] Prototype chain
     ...
     0} 들어가기에 앞서...
       ...
       11) 정의 : 특정 객체의 프로퍼티나 메소드에 접근하려고 할 때 해당 객체에 접근하려는 프로퍼티 또는 메소드가 없다면 [[Prototype]]이 가리키는 링크를 따라 자신의 부모 역할을 하는 프로토타입 객체의 프로퍼티나 메소드를 차례대로 검색하는 것
     ...
     1} 객체 리터럴 방식으로 생성된 객체의 프로토타입 체인
       ...
       13) 객체 리터럴 방식으로 생성된 객체 : 내장 함수(Built-in)인 Object() 생성자 함수로 객체를 생성하는 것을 단순화시킨 것
   ...
   7] 프로토타입 객체의 변경
     ...
     3} 프로토타입 객체 변경 시 발생하는 일
       ...
       7) 프로토타입 객체 변경 시점 이전에 생성된 객체 기존 프로토타입 객체를  [[Prototype]]에 바인딩한다.

       8) 프로토타입 객체 변경 시점 이후에 생성된 객체 변경된 프로토타입 객체를  [[Prototype]]에 바인딩한다.
...
8. 스코프
   ...
   2] 스코프의 구분
     ...
     2} 변수
       ...
       3) 전역 변수(Global variable) : 전역에서 선언된 변수이며 어디에든 참조할 수 있다.

       4) 지역 변수(Local variable) : 지역(함수) 내에서 선언된 변수이며 그 지역과 그 지역의 하부 지역에서만 참조할 수 있다.
   ...
   6] 함수 레벨 스코프(Function-level scope) 
     ...
     2} 전역변수 x와 지역변수 x가 중복 선언된 경우
       ...
       6) 내부 함수 : 전역변수는 물론 상위 함수에서 선언한 변수에 접근/변경이 가능하다.
