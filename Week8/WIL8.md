...
4. 연산자
   ...
   2] 문과 표현식
     ...
     2} 역할
       ...
       4) 문의 역할 : 표현식으로 생성한 값을 사용해 컴퓨터에게 명령을 내리는 것
   ...
   3] 연산자란?
     ...
     4} 연산자와 피연산자
       ...
       9) 연산자 : "값을 만든다"라는 동사의 역할
   
       10) 피연산자 : "값"이라는 명사의 역할

   4] 산술 연산자

     0} 들어가기에 앞서...

       11) 정의 : 피연산자를 대상으로 수학적 계산을 수행해 새로운 숫자 값을 만든다.
   
       12) 산술 연산을 할 수 없는 경우 : NaN을 반환하다.

     1} 이항 산술 연산자

       13) 연산 : 2개의 피연산자를 대상으로 연산하여 숫자 타입의 값을 만든다.

       14) 피연산자의 값을 변경하는 부수 효과(Side effect)가 없다.
     ...
     2} 단항 산술 연산자
       ...
       16) 연산 : 1개의 피연산자를 대상으로 연산한다.
   
       17) 피연산자의 값을 변경하는 부수 효과가 있다.
       ...
       19) 전위 증가/감소 연산자(Prefix increment/decrement operator) & 후위 증가/감소 연산자(Postfix increment/decrement operator)
     ...
     3} 문자열 연결 연산자
       ...
       20) + 연산자 : 피연산자 중 하나 이상이 문자열인 경우 문자열 연결 연산자로 동작한다.
       (그 외의 경우는 덧셈 연산자로 동작한다.)

       21) + / - 단항 연산자도 암묵적 타입 변환 발생이 가능하다.
   ...
   6] 비교 연산자(Comparison Operator)
     ...
     0} 들어가기에 앞서...
       ...
       18) 반환값 : 좌항과 우항의 피연산자를 비교하여 불리언 값을 반환한다.
   
       19) 사용 : 제어문(if 문, for 문 등)의 조건식에서 주로 사용한다.
     ...
     2} 대소 관계 비교 연산자
       ...
       21) 반환값 : 피연산자의 크기를 비교하여 불리언 값을 반환한다.
   ...
   7] 삼항 조건 연산자
     ...
     -> 조건식 ? 조건식이 true일 떄 반환할 값 : 조건식이 false일 때 반환할 값
       ...
       23) 조건식 : 물음표(?) 앞의 첫번째 피연산자, 불리언 타입의 값으로 평가될 표현식
   ...
   8] 논리 연산자(Logical Operator)
     ...
     3} 논리 부정(!) 연산자
       ...
       27) 반환값 : 언제나 불리언 값을 반환한다.

       28) 피연산자 : 반드시 불리언 값일 필요는 없다.
       -> 피연산자가 불리언 값이 아니면 불리언 타입으로 암묵적 타입 변환된다.

     4} 논리합(||) 연산자와 논리곱(&&) 연산자

       29) 연산 결과 : 일반적으로 불리언 값을 반환하지만 반드시 반환해야 하는 것은 아니다.

       30) 반환 : 언제나 피연산자 중 어느 한쪽 값을 반환한다.
   ...
   11] typeof 연산자
     ...
     2} typeof 연산자
       ...
       31) 반환 : 7가지 문자열("string", "number", "boolean", "undefined", "symbol", "object", "function"(함수)) 중 하나를 반환한다.
       -> "null"을 반환하는 경우는 없다.
       ...
       33) 주의사항 : 선언하지 않은 식별자를 typeof 연산자로 연산해보면 ReferenceError가 발생하지 않고 "undefined"를 반환한다.
...
7. 프로토타입
   ...
   4] Prototype chain
     ...
     1} 객체 리터럴 방식으로 생성된 객체의 프로토타입 체인
       ...
       14) 자바스크립트 엔진 : 객체 리터럴로 객체를 생성하는 코드를 만나면 내부적으로 Object() 생성자 함수(함수이므로 prototype 프로퍼티가 있다.)를 사용하여 객체를 생성한다.

       15) 객체 리터럴을 사용하여 객체를 생성한 경우, 그 객체의 프로토타입 객체는 Object.prototype이다.
   ...
   7] 프로토타입 객체의 변경
     ...
     3} 프로토타입 객체 변경 시 발생하는 일
       ...
       18) 프로토타입 객체 변경 시점 이전에 생성된 객체 기존 프로토타입 객체를  [[Prototype]]에 바인딩한다.

       19) 프로토타입 객체 변경 시점 이후에 생성된 객체 변경된 프로토타입 객체를  [[Prototype]]에 바인딩한다.
